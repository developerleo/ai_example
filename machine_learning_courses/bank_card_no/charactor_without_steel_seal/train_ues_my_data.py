...
def main():
    ...
    train_data, train_labels = IMG.img_handle('./images')
    IMG.data = []
    IMG.label = []
    ...
    train_labels = list(np.array(train_labels))

    #data = np.array(np.random.randint(0, 255, size=(3640, 16, 16)))
    #targets = list(np.random.randint(2, size=3640, dtype=np.int64))
    transform = transforms.Compose([transforms.ToTensor()])
    my_train_data = MyDataset(train_data, train_labels, transform=transform)

    validate_data, validate_labels = IMG.img_handle('./images_validation')
    my_validation_data = MyDataset(validate_data, validate_labels, transform=transform)

    net = TrainModule()
    # train_data = torchvision.datasets.CIFAR10(root='./dataset', train=True, transform=torchvision.transforms.ToTensor(), download=True)
    # train_data.data.shape = (50000, 32, 32, 3)
    # len(train_data.targets = 50000
    # test_data = torchvision.datasets.CIFAR10(root='./dataset', train=False, transform=torchvision.transforms.ToTensor(),download=True)
    # 查看数据集长度
    train_data_size = len(my_train_data)
    test_data_size = len(my_validation_data)
    print('训练数据集长度为：{}'.format(train_data_size))
    print('测试数据集长度为：{}'.format(test_data_size))

    train_dataloader = DataLoader(my_train_data, batch_size=16)
    test_dataloader = DataLoader(my_validation_data, batch_size=16)
    ...
    for i in range(epoch):
        ...
        torch.save(net, 'model/tarin_{}.pth'.format(total_test_step))
